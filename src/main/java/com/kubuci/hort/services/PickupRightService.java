...
public class PickupRightService {
	...
	public List<PermissionViewDto> listPermissions(String statusFilter) {
		...
		List<PickupRight> pickupRights =
			statusFilter.equalsIgnoreCase("ALL") ? repo.findAllWithStudentAndCollector() : repo.findByStatusWithStudentAndCollector(PermissionStatus.ACTIVE);

		// 2. Buscar SelfDismissal
		List<SelfDismissal> selfDismissals = statusFilter.equalsIgnoreCase("ALL") ? selfDismissalRepository.findAllWithStudent()
		...
		for (PickupRight pr : pickupRights) {
			var s = pr.getStudent();
			var sp = s.getPerson();
			var gName = s.getGroup() != null ? s.getGroup().getName() : null;

			var c = pr.getCollector();
			var cp = c.getPerson();

			list.add(new PermissionViewDto(
				pr.getId(),
				"COLLECTOR",
				s.getId(),
				sp.getFirstName(),
				...
				cp.getLastName(),
				cp.getPhone(),
				pr.getValidFrom() != null ? pr.getValidFrom().toString() : null,
				pr.getValidUntil() != null ? pr.getValidUntil().toString() : null,
				pr.getAllowedFromTime() != null ? pr.getAllowedFromTime().toString() : null,
				pr.getStatus().name()
			));
		}
		...
		for (SelfDismissal sd : selfDismissals) {
			...
			list.add(new PermissionViewDto(
				...
				null,               // collectorPhone
				sd.getValidFrom() != null ? sd.getValidFrom().toString() : null,
				sd.getValidUntil() != null ? sd.getValidUntil().toString() : null,
				sd.getAllowedFromTime() != null ? sd.getAllowedFromTime().toString() : null,
				sd.getStatus().name()
			));
		}
		...
		list.sort((a, b) -> {
			...
			return b.validFrom().compareToIgnoreCase(a.validFrom());
		});
	...
	public void createPermission(NewPermissionRequest req) {
		...
		LocalDateTime validFrom = req.validFrom() != null ? req.validFrom() : now;
	...
	private void createSelfDismissal(
		Student student,
		NewPermissionRequest req,
		LocalDateTime validFrom,
		LocalDateTime validUntil
	) {
		// allowedFromTime:
		// - Tagesvollmacht: req.allowedFromTime es una sola hora en ese día
		// - Dauervollmacht con weeklyAllowedFrom: OJO esto no cabe directo en nuestro modelo actual
		//
		// Hoy la tabla `self_dismissal` sólo tiene UNA `allowed_from_time`.
		// Eso significa que para el caso "DAUER + canLeaveAlone + horario distinto por día"
		// hay que definir:
		//
		//  Opción simple (MVP): en este release guardamos un único fromTime común (ej. el más temprano de la semana).
		//  Opción larga: crear otra tabla tipo self_dismissal_weekday con day_of_week + allowed_from_time.
		//
		...
		if (req.kind().equalsIgnoreCase("TAGES")) {
	...
	private void createPickupRight(
		Student student,
		NewPermissionRequest req,
		LocalDateTime validFrom,
		LocalDateTime validUntil
		...
		pr.setStatus(PermissionStatus.ACTIVE);
		pr.setType(
			req.kind().equalsIgnoreCase("TAGES")
